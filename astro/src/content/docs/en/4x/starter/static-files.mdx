...
For more information on the `options` argument, see [express.static](/api/application/expressstatic).
...
To create a virtual path prefix (where the path does not actually exist in the file system) for files that are served by the `express.static` function, [specify a mount path](/api/application/app-use/) for the static directory, as shown below:
