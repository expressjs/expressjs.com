...
Para uma introdução ao roteamento, veja [Roteamento básico](/en/4x/starter/basic-routing).
...
ver [app.METHOD](/en/4x/api/application#appmethodpath-callback--callback-). Você também pode usar o [app.all()](/en/4x/api/application#appallpath-callback--callback-) para lidar com todos os métodos HTTP e [app. se()](/en/4x/api/application#appusepath-callback--callback) para
especifique middleware como função de retorno de chamada (Veja [Usando middleware](/en/4x/guide/using-middleware) para detalhes).
...
Para uma lista completa, consulte [app.METHOD](/en/4x/api/application#appmethodpath-callback--callback-).
...
| Método                                                                       | Descrição:                                                                                          |
| ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| [res.download()](/en/4x/api/response#resdownloadpath--filename--options--fn) | Solicite que um arquivo seja baixado.                                                               |
| [res.end()](/en/4x/api/response#resenddata-encoding-callback)                | Encerrar o processo de resposta.                                                                    |
| [res.json()](/en/4x/api/response#resjsonbody)                                | Enviar uma resposta JSON.                                                                           |
| [res.jsonp()](/en/4x/api/response#resjsonpbody)                              | Envie uma resposta JSON com suporte a JSONP.                                                        |
| [res.redirect()](/en/4x/api/response#resredirectstatus-path)                 | Redirecionar uma requisição.                                                                        |
| [res.render()](/en/4x/api/response#resrenderview--locals--callback)          | Renderizar um modelo de visão.                                                                      |
| [res.send()](/en/4x/api/response#ressendbody)                                | Envie uma resposta de vários tipos.                                                                 |
| [res.sendFile()](/en/4x/api/response#ressendfilepath--options--fn)           | Envia um arquivo como uma transmissão octet.                                                        |
| [res.sendStatus()](/en/4x/api/response#ressendstatusstatuscode)              | Defina o código de status da resposta e envie sua representação de string como o corpo da resposta. |

## app.route()

Você pode criar manipuladores de rotas em cadeia para um caminho de rota usando `app.route()`.
Como o caminho é especificado em um único local, é útil criar rotas modulares, assim como reduzir a redundância e os tipos. Para obter mais informações sobre rotas, consulte: [documentação de roteador](/en/4x/api/router).
...
Mas se a rota pai `/birds` tiver parâmetros de caminho, ela não será acessível por padrão nas sub-rotas. Para torná-lo acessível, você precisará passar a opção `mergeParams` para o construtor do roteador [reference](/en/4x/api/application#appusepath-callback--callback).
