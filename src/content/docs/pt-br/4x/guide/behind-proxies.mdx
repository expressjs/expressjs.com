...
<ul>
  ...
  <li markdown="1">
    `X-Forwarded-Proto` pode ser configurado pelo proxy reverso para dizer ao aplicativo se ele é `https` ou
    `http` ou até mesmo um nome inválido. Este valor é refletido por [req.protocol](/en/api#req.protocol).
  </li>
  <li markdown="1">
    Os valores [req.ip](/en/api#req.ip) e [req.ips](/en/api#req.ips) são preenchidos com base no cabeçalho
    do socket e `X-Forwarded-For`, iniciando no primeiro endereço não confiável.
  </li>
</ul>
