...
ver [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-). Você também pode usar o [app.all()](/en/5x/api/application#appallpath-callback--callback-) para lidar com todos os métodos HTTP e [app. se()](/en/5x/api/application#appusepath-callback--callback) para
...
Para uma lista completa, consulte [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-).
...
Caracteres Regexp não são suportados nos caminhos da rota. Use uma matriz de caminhos ou expressões regulares em vez disso.
Veja a [sintaxe correspondente à trajetória](/en/guide/migrating-5#path-syntax) para obter mais informações.
...
| Método                                                                       | Descrição:                                                                                          |
| ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| [res.download()](/en/5x/api/response#resdownloadpath--filename--options--fn) | Solicite que um arquivo seja baixado.                                                               |
| [res.end()](/en/5x/api/response#resenddata-encoding-callback)                | Encerrar o processo de resposta.                                                                    |
| [res.json()](/en/5x/api/response#resjsonbody)                                | Enviar uma resposta JSON.                                                                           |
| [res.jsonp()](/en/5x/api/response#resjsonpbody)                              | Envie uma resposta JSON com suporte a JSONP.                                                        |
| [res.redirect()](/en/5x/api/response#resredirectstatus-path)                 | Redirecionar uma requisição.                                                                        |
| [res.render()](/en/5x/api/response#resrenderview--locals--callback)          | Renderizar um modelo de visão.                                                                      |
| [res.send()](/en/5x/api/response#ressendbody)                                | Envie uma resposta de vários tipos.                                                                 |
| [res.sendFile()](/en/5x/api/response#ressendfilepath--options--fn)           | Envia um arquivo como uma transmissão octet.                                                        |
| [res.sendStatus()](/en/5x/api/response#ressendstatusstatuscode)              | Defina o código de status da resposta e envie sua representação de string como o corpo da resposta. |
...
Mas se a rota pai `/birds` tiver parâmetros de caminho, ela não será acessível por padrão nas sub-rotas. Para torná-lo acessível, você precisará passar a opção `mergeParams` para o construtor do roteador [reference](/en/5x/api/application#appusepath-callback--callback).
