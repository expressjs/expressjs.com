...
app.set('trust proxy', (ip) => {
  if (ip === '127.0.0.1' || ip === '123.123.123. 23')
    retourne true; // les adresses IP
  approuvées retournent false;
});
```
...
<ul>
  <li markdown="1">
    La valeur de [req.hostname](/en/api#req.hostname) est dérivée de la valeur définie dans l'en-tête
    `X-Forwarded-Host`, qui peut être définie par le client ou par le proxy.
  </li>
  <li markdown="1">
    `X-Forwarded-Proto` peut être défini par le proxy inverse pour dire à l'application si c'est `https` ou
    `http` ou même un nom invalide. Cette valeur est reflétée par [req.protocol](/en/api#req.protocol).
  </li>
  <li markdown="1">
    Les valeurs [req.ip](/en/api#req.ip) et [req.ips](/en/api#req.ips) sont remplies en fonction de l'adresse du socket
    et de l'en-tête `X-Forwarded-For`, à partir de la première adresse non fiable.
  </li>
</ul>
