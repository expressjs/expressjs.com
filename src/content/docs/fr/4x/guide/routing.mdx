...
Pour une introduction au routage, voir [Routage de base](/en/4x/starter/basic-routing).
...
voir [app.METHOD](/en/4x/api/application#appmethodpath-callback--callback-). Vous pouvez également utiliser [app.all()](/en/4x/api/application#appallpath-callback--callback-) pour gérer toutes les méthodes HTTP et [app. se()](/en/4x/api/application#appusepath-callback--callback) à
spécifient le middleware comme fonction de callback (Voir [Utilisation du middleware](/en/4x/guide/using-middleware) pour plus de détails).
...
Pour une liste complète, voir [app.METHOD](/en/4x/api/application#appmethodpath-callback--callback-).
...
<Alert type="avertissement">
Le caractère <a href="https://github.com/expressjs/express/issues/2495">`*` dans les expressions régulières n'est pas interprété de la manière habituelle</a>. Comme solution de contournement, utilisez `{0,}` au lieu de `*`.
</Alert>
...
| Méthode                                                                      | Libellé                                                                                           |
| ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| [res.download()](/en/4x/api/response#resdownloadpath--filename--options--fn) | Demander au téléchargement un fichier.                                                            |
| [res.end()](/en/4x/api/response#resenddata-encoding-callback)                | Terminer le processus de réponse.                                                                 |
| [res.json()](/en/4x/api/response#resjsonbody)                                | Envoyer une réponse JSON.                                                                         |
| [res.jsonp()](/en/4x/api/response#resjsonpbody)                              | Envoyer une réponse JSON avec le support JSONP.                                                   |
| [res.redirect()](/en/4x/api/response#resredirectstatus-path)                 | Rediriger une requête.                                                                            |
| [res.render()](/en/4x/api/response#resrenderview--locals--callback)          | Afficher un modèle de vue.                                                                        |
| [res.send()](/en/4x/api/response#ressendbody)                                | Envoyer une réponse de différents types.                                                          |
| [res.sendFile()](/en/4x/api/response#ressendfilepath--options--fn)           | Envoyer un fichier en tant que flux octet.                                                        |
| [res.sendStatus()](/en/4x/api/response#ressendstatusstatuscode)              | Définit le code de statut de la réponse et envoie sa représentation en tant que corps de réponse. |

## app.route()

Vous pouvez créer des gestionnaires de routes chaînables pour un chemin en utilisant `app.route()`.
Parce que le chemin est spécifié à un seul endroit, la création de routes modulaires est utile, tout comme la réduction de la redondance et des fautes de frappe. Pour plus d'informations sur les routes, voir : [Documentation de Router()](/en/4x/api/router).
...
Mais si la route parente `/birds` a des paramètres de chemin, elle ne sera pas accessible par défaut à partir des sous-routes. Pour le rendre accessible, vous devrez passer l'option `mergeParams` au constructeur de routeur [reference](/en/4x/api/application#appusepath-callback--callback).
