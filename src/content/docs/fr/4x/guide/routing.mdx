...
voir [app.METHOD](/api/application#appmethodpath-callback--callback-). Vous pouvez également utiliser [app.all()](/api/application#appallpath-callback--callback-) pour gérer toutes les méthodes HTTP et [app. se()](/api/application#appusepath-callback--callback) à
spécifient le middleware comme fonction de callback (Voir [Utilisation du middleware](/guide/using-middleware) pour plus de détails).
...
Pour une liste complète, voir [app.METHOD](/api/application#appmethodpath-callback--callback-).
...
| Méthode                                                                | Libellé                                                                                           |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| [res.download()](/api/response#resdownloadpath--filename--options--fn) | Demander au téléchargement un fichier.                                                            |
| [res.end()](/api/response#resenddata-encoding-callback)                | Terminer le processus de réponse.                                                                 |
| [res.json()](/api/response#resjsonbody)                                | Envoyer une réponse JSON.                                                                         |
| [res.jsonp()](/api/response#resjsonpbody)                              | Envoyer une réponse JSON avec le support JSONP.                                                   |
| [res.redirect()](/api/response#resredirectstatus-path)                 | Rediriger une requête.                                                                            |
| [res.render()](/api/response#resrenderview--locals--callback)          | Afficher un modèle de vue.                                                                        |
| [res.send()](/api/response#ressendbody)                                | Envoyer une réponse de différents types.                                                          |
| [res.sendFile()](/api/response#ressendfilepath--options--fn)           | Envoyer un fichier en tant que flux octet.                                                        |
| [res.sendStatus()](/api/response#ressendstatusstatuscode)              | Définit le code de statut de la réponse et envoie sa représentation en tant que corps de réponse. |
...
Mais si la route parente `/birds` a des paramètres de chemin, elle ne sera pas accessible par défaut à partir des sous-routes. Pour le rendre accessible, vous devrez passer l'option `mergeParams` au constructeur de routeur [reference](/api/application#appusepath-callback--callback).
