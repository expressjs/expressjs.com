...
Les fonctions *Middleware* sont des fonctions qui ont accès à l'objet [request object](/en/5x/api#req) (`req`), l'objet [réponse](/en/5x/api#res) (`res`), et la fonction `next` dans le cycle de réponse de l'application. La fonction `next` est une fonction dans le routeur Express qui, lorsqu'elle est appelée, exécute le middleware qui succède au middleware actuel.
...
<div class="callout" id="callout5">
  L'argument HTTP <a href="/en/5x/api#res">réponse</a> à la fonction middleware appelée "res" par convention
  .
</div>

<div class="callout" id="callout6">HTTP <a href="/en/5x/api#req">demande</a> argument à la fonction middleware appelée "req" par convention.</div>
</td>
</tr>
</table>
</div>
...
app. et('/', (req, res) => {
  let responseText = 'Bonjour le monde!<br>';
  responseText += `<small>Demandé à: ${req.requestTime}</small>`;
...
```js
fonction async cookieValidator(cookies) {
  essayer {
    attendre externalyValidateCookie(cookies.testCookie);
  } catch {
    throw new Error('Cookies invalides');
  }
}
```

Ici, nous utilisons le middleware [`cookie-parser`](/en/resources/middleware/cookie-parser) pour analyser les cookies entrants de l'objet `req` et les passer à notre fonction `cookieValidator`. Le middleware `validateCookies` retourne une Promesse que si le rejet déclenchera automatiquement notre gestionnaire d'erreurs.
...
fonction async validateCookies(req, res, next) {
  attendent cookieValidator(req. ookies);
  next();
}
...
Pour plus d'informations sur le middleware Express, voir : [Utiliser un middleware Express](/en/guide/using-middleware).
...
module. xports = function (options) {
  fonction de retour (req, res, next) {
    // Implémente la fonction middleware basée sur l'objet d'options
    next();
  };
};
```
