...
Les fonctions _Middleware_ sont des fonctions qui ont accès à l'objet [request object](/en/5x/api#req) (`req`), l'objet [réponse](/en/5x/api#res) (`res`), et la fonction `next` dans le cycle de réponse de l'application. La fonction `next` est une fonction dans le routeur Express qui, lorsqu'elle est appelée, exécute le middleware qui succède au middleware actuel.
...
<div class="callout" id="callout5">
  L'argument HTTP <a href="/en/5x/api#res">réponse</a> à la fonction middleware appelée "res" par
  convention .
</div>
