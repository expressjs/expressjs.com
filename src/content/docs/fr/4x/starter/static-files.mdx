...
Pour plus d'informations sur l'argument `options`, voir [express.static](/5x/api#express.static).
...
Pour créer un préfixe de chemin virtuel (où le chemin n'existe pas dans le système de fichiers) pour les fichiers qui sont servis par le fichier `express. fonction tatic`, [spécifiez un chemin de montage](/5x/api#app.use) pour le répertoire statique, comme indiqué ci-dessous:
...
Pour plus de détails sur la fonction `serve-static` et ses options, voir [serve-static](/resources/middleware/serve-static).
