...
voir [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-). Vous pouvez également utiliser [app.all()](/en/5x/api/application#appallpath-callback--callback-) pour gérer toutes les méthodes HTTP et [app. se()](/en/5x/api/application#appusepath-callback--callback) à
...
Pour une liste complète, voir [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-).
...
Les caractères Regexp ne sont pas pris en charge dans les chemins de route. Utilisez un tableau de chemins ou d'expressions régulières à la place.
...
| Méthode                                                                      | Libellé                                                                                           |
| ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| [res.download()](/en/5x/api/response#resdownloadpath--filename--options--fn) | Demander au téléchargement un fichier.                                                            |
| [res.end()](/en/5x/api/response#resenddata-encoding-callback)                | Terminer le processus de réponse.                                                                 |
| [res.json()](/en/5x/api/response#resjsonbody)                                | Envoyer une réponse JSON.                                                                         |
| [res.jsonp()](/en/5x/api/response#resjsonpbody)                              | Envoyer une réponse JSON avec le support JSONP.                                                   |
| [res.redirect()](/en/5x/api/response#resredirectstatus-path)                 | Rediriger une requête.                                                                            |
| [res.render()](/en/5x/api/response#resrenderview--locals--callback)          | Afficher un modèle de vue.                                                                        |
| [res.send()](/en/5x/api/response#ressendbody)                                | Envoyer une réponse de différents types.                                                          |
| [res.sendFile()](/en/5x/api/response#ressendfilepath--options--fn)           | Envoyer un fichier en tant que flux octet.                                                        |
| [res.sendStatus()](/en/5x/api/response#ressendstatusstatuscode)              | Définit le code de statut de la réponse et envoie sa représentation en tant que corps de réponse. |
...
Mais si la route parente `/birds` a des paramètres de chemin, elle ne sera pas accessible par défaut à partir des sous-routes. Pour le rendre accessible, vous devrez passer l'option `mergeParams` au constructeur de routeur [reference](/en/5x/api/application#appusepath-callback--callback).
