...
return a Promise, so their errors reach Express with no extra work. Par exemple :
...
Express will catch and process them. Par exemple :
...
call. Si `readFile` cause une erreur, alors il passe l'erreur à Express, sinon vous
...
D'autres erreurs de gestion du middleware peuvent être trouvées dans [middleware Express](/resources/middleware).
...
Notice that when you do not call `next` in an error-handling function, you are responsible for writing (and ending) the response. Sinon, ces demandes seront « bloquées » et ne seront pas admissibles au ramassage des déchets.
