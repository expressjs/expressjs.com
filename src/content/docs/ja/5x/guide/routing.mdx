...
完全なリストについては、 [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-) を参照してください。
...
詳細については、[パスルートマッチング構文](/en/guide/migrating-5#path-syntax)を参照してください。
...
ただし、親ルート `/birds` にパスパラメータがある場合、サブルートからデフォルトではアクセスできません。 アクセス可能にするには、 `mergeParams` オプションを Router コンストラクタ [reference](/en/5x/api/application#appusepath-callback--callback) に渡す必要があります。
