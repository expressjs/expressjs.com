...
完全なリストについては、 [app.METHOD](/en/4x/api/application#appmethodpath-callback--callback-) を参照してください。
...
<Alert type="警告">
The <a href="https://github.com/expressjs/express/issues/2495">`*` character in regular expressions is not interpreted in the usual way</a>. 回避策として、`*` の代わりに `{0,}` を使用します。
</Alert>
...
パスは単一の場所で指定されているため、モジュラールートを作成することは、冗長性とタイプミスを削減するのに役立ちます。 ルートの詳細については、以下を参照してください: [Router() documentation](/en/4x/api/router)。
...
ただし、親ルート `/birds` にパスパラメータがある場合、サブルートからデフォルトではアクセスできません。 アクセス可能にするには、 `mergeParams` オプションを Router コンストラクタ [reference](/en/4x/api/application#appusepath-callback--callback) に渡す必要があります。
