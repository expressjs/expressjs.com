...
siehe [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-). Du kannst auch [app.all()](/en/5x/api/application#appallpath-callback--callback-) verwenden, um alle HTTP-Methoden und [app. se()](/en/5x/api/application#appusepath-callback--callback) bis
...
Für eine vollständige Liste siehe [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-).
...
| ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| [res.download()](/en/5x/api/response#resdownloadpath--filename--options--fn) | Fordern Sie eine Datei zum Download an.                                                                   |
| [res.end()](/en/5x/api/response#resenddata-encoding-callback)                | Beenden Sie den Antwort-Prozess.                                                                          |
| [res.json()](/en/5x/api/response#resjsonbody)                                | Sende eine JSON-Antwort.                                                                                  |
| [res.jsonp()](/en/5x/api/response#resjsonpbody)                              | Senden Sie eine JSON-Antwort mit JSONP-Unterstützung.                                                     |
| [res.redirect()](/en/5x/api/response#resredirectstatus-path)                 | Anfrage umleiten.                                                                                         |
| [res.render()](/en/5x/api/response#resrenderview--locals--callback)          | Ansichtsvorlage ausblenden.                                                                               |
| [res.send()](/en/5x/api/response#ressendbody)                                | Senden Sie eine Antwort von verschiedenen Typen.                                                          |
| [res.sendFile()](/en/5x/api/response#ressendfilepath--options--fn)           | Senden Sie eine Datei als octet-Stream.                                                                   |
| [res.sendStatus()](/en/5x/api/response#ressendstatusstatuscode)              | Legen Sie den Antwort-Statuscode fest und senden Sie seine Zeichenfolge Repräsentation als Antwortkörper. |
...
Aber wenn die übergeordnete Route `/birds` Pfadparameter hat, wird sie standardmäßig nicht von den Unterrouten aus erreichbar sein. Um es zugänglich zu machen, müssen Sie die Option "mergeParams" an den Router-Konstruktor [reference](/en/5x/api/application#appusepath-callback--callback) übergeben.
