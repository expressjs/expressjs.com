...
Für weitere Informationen zum Argument `options` siehe [express.static](/5x/api#express.static).
...
Um einen virtuellen Pfad-Präfix zu erstellen (wo der Pfad existiert eigentlich nicht im Dateisystem) für Dateien, die durch die `express. tatic` Funktion, [einen Mount-Pfad angeben](/5x/api#app.use) für das statische Verzeichnis angeben, wie unten angezeigt:
