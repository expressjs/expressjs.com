...
<div class="tabell-scroller">
<table id="mw-Abbildung">
<tr>
<td id="mw-fig-imgcell">
<img src="/images/express-mw.png" alt="Elemente eines Middleware Funktionsaufrufs" id="mw-fig-img" />
</td>
<td class="mw-fig-callouts">
<div class="callout" id="callout1">HTTP-Methode, für die die Middleware-Funktion gilt.</div>
...
<div class="callout" id="callout5">
  HTTP <a href="/en/5x/api#res">Antwort</a> Argument auf die Middleware-Funktion, genannt "res" von
  Konvention.
</div>

<div class="callout" id="callout6">HTTP <a href="/en/5x/api#req">Request</a> Argument an die Middleware-Funktion, genannt "req" von Konvention.</div>
</td>
</tr>
</table>
</div>
...
async function cookieValidator(cookies) {
  try {
    wait externallyValidateCookie(cookies.testCookie);
  } catch {
    throw new Error('Ungültige Cookies');
  }
}
```
...
async function validateCookies(req, res, next) {
  wartet cookieValidator(req. ookies);
  next();
}
...
```js
modul.xports = function (options) {
  return function (req, res, next) {
    // Implementierung der Middleware-Funktion basierend auf dem Options-Objekt
    next();
  };
};
```
