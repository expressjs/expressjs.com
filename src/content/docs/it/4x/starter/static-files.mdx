...
Per ulteriori informazioni sull'argomento `options`, vedere [express.static](/api#express.static).
...
Per creare un prefisso di percorso virtuale (dove il percorso non esiste effettivamente nel file system) per i file serviti dal `express. funzione tatic`, [specify a mount path](/api#app.use) per la directory statica, come mostrato di seguito:
