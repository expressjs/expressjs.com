...
Per ulteriori informazioni sull'argomento `options`, vedere [express.static](/5x/api#express.static).
...
Per creare un prefisso di percorso virtuale (dove il percorso non esiste effettivamente nel file system) per i file serviti dal `express. funzione tatic`, [specify a mount path](/5x/api#app.use) per la directory statica, come mostrato di seguito:
...
Per maggiori dettagli sulla funzione `serve-static` e sulle sue opzioni, vedere [serve-static](/resources/middleware/serve-static).
