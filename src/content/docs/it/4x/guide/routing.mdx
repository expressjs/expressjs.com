...
Per un'introduzione al routing, vedere [Instradamento di base](/en/4x/starter/basic-routing).
...
vedi [app.METHOD](/en/4x/api/application#appmethodpath-callback--callback-). Puoi anche usare [app.all()](/en/4x/api/application#appallpath-callback--callback-) per gestire tutti i metodi HTTP e [app. se()](/en/4x/api/application#appusepath-callback--callback) to
specify middleware as the callback function (Vedi [Using middleware](/en/4x/guide/using-middleware) for details).
...
Per un elenco completo, vedere [app.METHOD](/en/4x/api/application#appmethodpath-callback--callback-).
...
<Alert type="avviso">
Il carattere `*` <a href="https://github.com/expressjs/express/issues/2495">nelle espressioni regolari non è interpretato nel modo usuale</a>. Come soluzione puoi usare `{0,}` invece di `*`.
</Alert>
...
| Metodo                                                                       | Descrizione                                                                                                        |
| ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| [res.download()](/en/4x/api/response#resdownloadpath--filename--options--fn) | Avverte un file da scaricare.                                                                                      |
| [res.end()](/en/4x/api/response#resenddata-encoding-callback)                | Termina il processo di risposta.                                                                                   |
| [res.json()](/en/4x/api/response#resjsonbody)                                | Invia una risposta JSON.                                                                                           |
| [res.jsonp()](/en/4x/api/response#resjsonpbody)                              | Invia una risposta JSON con il supporto JSONP.                                                                     |
| [res.redirect()](/en/4x/api/response#resredirectstatus-path)                 | Reindirizza una richiesta.                                                                                         |
| [res.render()](/en/4x/api/response#resrenderview--locals--callback)          | Render un modello di visualizzazione.                                                                              |
| [res.send()](/en/4x/api/response#ressendbody)                                | Invia una risposta di vari tipi.                                                                                   |
| [res.sendFile()](/en/4x/api/response#ressendfilepath--options--fn)           | Invia un file come un flusso di otte.                                                                              |
| [res.sendStatus()](/en/4x/api/response#ressendstatusstatuscode)              | Imposta il codice di stato della risposta e invia la sua rappresentazione della stringa come corpo della risposta. |

## app.route()

È possibile creare i gestori di rotte per un percorso utilizzando `app.route()`.
Poiché il percorso è specificato in una singola posizione, è utile creare percorsi modulari, così come ridurre la ridondanza e pneumatici. Per ulteriori informazioni sulle rotte, si veda: [Router() documentation](/en/4x/api/router).
...
Ma se il percorso principale `/birds` ha parametri di percorso, non sarà accessibile per impostazione predefinita dai sotto-percorsi. Per renderlo accessibile, dovrai passare l'opzione `mergeParams` al costruttore Router [reference](/en/4x/api/application#appusepath-callback--callback).
