...
Le funzioni _Middleware_ sono funzioni che hanno accesso al [request object](/5x/api#req) (`req`), il [response object](/5x/api#res) (`res`), e la successiva funzione middleware nel ciclo request-response dell'applicazione. La funzione middleware successiva è comunemente indicata da una variabile chiamata `next`.
...
Associa il middleware di livello applicativo ad un'istanza del [app object](/5x/api#app) usando `app.use()` e `app. ETHOD()` funzioni, dove `METHOD` è il metodo HTTP della richiesta che la funzione middleware gestisca (come GET, PUT, o POST) in minuscolo.
...
- [express.static](/5x/api#express.static) serve risorse statiche come file HTML, immagini e così via.
- [express.json](/5x/api#express.json) analizza le richieste in arrivo con i payload JSON. **NOTA: Disponibile con Express 4.16.0+**
- [express.urlencoded](/5x/api#express.urlencoded) analizza le richieste in arrivo con payload codificati con URL. **NOTA: Disponibile con Express 4.16.0+**
