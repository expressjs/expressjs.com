...
Le funzioni _Middleware_ sono funzioni che hanno accesso al [request object](/en/5x/api#req) (`req`), il [response object](/en/5x/api#res) (`res`), e la funzione `next` nel ciclo request-response dell'applicazione. La funzione `next` è una funzione nel router Express che, quando invocato, esegue il middleware con successo al middleware corrente.
...
<div class="callout" id="callout5">
  Argomento HTTP <a href="/en/5x/api#res">risposta</a> alla funzione middleware, chiamato "res"
  dalla convenzione .
</div>
