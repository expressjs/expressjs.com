...
vea [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-). También puedes usar [app.all()](/en/5x/api/application#appallpath-callback--callback-) para manejar todos los métodos HTTP y [app. se()](/en/5x/api/application#appusepath-callback--callback) a
...
Para una lista completa, vea [app.METHOD](/en/5x/api/application#appmethodpath-callback--callback-).
...
Los caracteres `?`, `+`, `*`, `[]`, y `()` están reservados y no pueden usarse como caracteres literales en rutas de ruta. Usa `\` para escapar de ellos si es necesario.
...
Los caracteres de expresión regular no están soportados en las rutas de ruta. Utilice un array de rutas o expresiones regulares en su lugar.
Vea la [sintaxis de coincidencia de ruta de ruta](/en/guide/migrating-5#path-syntax) para más información.
...
| Método                                                                       | Descripción                                                                                                 |
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| [res.download()](/en/5x/api/response#resdownloadpath--filename--options--fn) | Solicitar un archivo para ser descargado.                                                                   |
| [res.end()](/en/5x/api/response#resenddata-encoding-callback)                | Terminar el proceso de respuesta.                                                                           |
| [res.json()](/en/5x/api/response#resjsonbody)                                | Enviar una respuesta JSON.                                                                                  |
| [res.jsonp()](/en/5x/api/response#resjsonpbody)                              | Envía una respuesta JSON con soporte JSONP.                                                                 |
| [res.redirect()](/en/5x/api/response#resredirectstatus-path)                 | Redirigir una solicitud.                                                                                    |
| [res.render()](/en/5x/api/response#resrenderview--locals--callback)          | Procesar una plantilla de vista.                                                                            |
| [res.send()](/en/5x/api/response#ressendbody)                                | Enviar una respuesta de varios tipos.                                                                       |
| [res.sendFile()](/en/5x/api/response#ressendfilepath--options--fn)           | Enviar un archivo como un flujo de octet.                                                                   |
| [res.sendStatus()](/en/5x/api/response#ressendstatusstatuscode)              | Establece el código de estado de respuesta y envía su representación de cadena como el cuerpo de respuesta. |
...
Pero si la ruta padre `/birds` tiene parámetros de ruta, no será accesible por defecto desde las subrutas. Para hacerlo accesible, necesitarás pasar la opción `mergeParams` al constructor de Router [reference](/en/5x/api/application#appusepath-callback--callback).
