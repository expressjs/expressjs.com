...
Las funciones _Middleware_ son funciones que tienen acceso al [objeto de solicitud](/5x/api#req) (`req`), el [objeto de respuesta](/5x/api#res) (`res`), y la función `next` en el ciclo de solicitud y respuesta de la aplicación. La función `siguiente` es una función en el router Express que, cuando se invoca, ejecuta el middleware sucediendo al middleware actual.
...
<div class="callout" id="callout5">
  HTTP <a href="/5x/api#res">argumento</a> de respuesta a la función de middleware, llamada "res"
  por convención .
</div>

<div class="callout" id="callout6">HTTP <a href="/5x/api#req">solicita un argumento</a> a la función de middleware, llamado "req" por convención.</div>
</td>
</tr>
</table>
</div>
...
Aquí usamos el [`cookie-parser`](/resources/middleware/cookie-parser) middleware para analizar las cookies entrantes del objeto `req` y pasarlas a nuestra función `cookieValidator`. El middleware `validateCookies` devuelve una Promesa que al rechazar activará automáticamente nuestro gestor de errores.
...
Para más información sobre middleware exprés, vea: [Usando middleware exprés](/guide/using-middleware).
