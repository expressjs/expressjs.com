...
| `http.createServer()`              | The `http` module is no longer needed, unless you need to directly work with it (socket.io/SPDY/HTTPS). 可通过 `app.listen()` 函数启动应用。 |
| `app.configure()`                  | `app.configure()` 函数已被删除。 使用 `process.env.NODE_ENV` 或&#xA;`app.get('env)` 函数检测环境并相应配置应用程序。                         |
| `json spaces`                      | Express 4默认禁用了 "json spaces" 应用程序属性。                                                                                             |
| `req.accepted()`                   | 使用`req.accepts()`, `req.acceptsEncodings()`, `req.acceptsCharsets()`, 和&#xA;`req.acceptsLanguages()`.                                     |
...
| `res.charset`                      | Removed.                                                                                                                                     |
| `res.setHeader('Set-Cookie', val)` | 功能现已受限，仅用于设置基础的 Cookie 值。 Use `res.cookie()` for added functionality.                                                       |
