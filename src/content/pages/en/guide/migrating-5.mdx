...
Several MIME types have changed due to updates in [`mime-db`](https://github.com/jshttp/mime-db). This affects `express.static()`, `res.sendFile()`, and any other method that relies on MIME type detection. For a full list of changes, see the [`mime-db` changelog](https://github.com/jshttp/mime-db/blob/master/HISTORY.md):
