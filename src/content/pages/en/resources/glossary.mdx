...
When used as an adjective, hyphenate; for example: "This is open-source software." See [Open-source software on Wikipedia](https://en.wikipedia.org/wiki/Open-source_software).
...
An HTTP request. A client submits an HTTP request message to a server, which returns a response. The request must use one of several [request methods](https://en.wikipedia.org/wiki/HTTP#Request_methods) such as GET, POST, and so on.
...
See [router](/api/router) in the API reference.
