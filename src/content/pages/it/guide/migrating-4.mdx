...
| `http.createServer()`              | The `http` module is no longer needed, unless you need to directly work with it (socket.io/SPDY/HTTPS). L'app può essere avviata utilizzando la funzione `app.listen()`. |
| `app.configure()`                  | La funzione `app.configure()` è stata rimossa. Usa la funzione `process.env.NODE_ENV` o&#xA;`app.get('env')` per rilevare l'ambiente e configurare l'app di conseguenza. |
| `json spaces`                      | The `json spaces` application property is disabled by default in Express 4.                                                                                              |
| `req.accepted()`                   | Usa `req.accepts()`, `req.acceptsEncodings()`, `req.acceptsCharsets()`, and&#xA;`req.acceptsLanguages()`.                                                                |
...
| `res.charset`                      | Removed.                                                                                                                                                                 |
| `res.setHeader('Set-Cookie', val)` | La funzionalità è ora limitata all'impostazione del valore di base dei cookie. Use `res.cookie()` for added functionality.                                               |
