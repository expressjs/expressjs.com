...
Un esempio di input utente potenzialmente pericoloso è un _open redirect_, dove un'applicazione accetta un URL come input utente (spesso nella domanda URL, ad esempio `?url=https://example.com`) e utilizza `res.redirect` per impostare l'intestazione `location` e
