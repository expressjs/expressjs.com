...
`req.session`の
内容が変更された場合、ミドルウェアは自動的に`Set-Cookie`ヘッダーをレスポンスに追加します。 *Note* that no `Set-Cookie` header will be
...
* `secure`: CookieがHTTPSでのみ送信されるかどうかを示すブール値 (HTTPでは`false`、HTTPSではデフォルトでは`true`)。 `true` と Node に設定されている場合。 sはTLS接続上で直接ではありません。 [set Express behind proxies](https://expressjs.com/en/guide/behind-proxies.html)またはクッキーが正しく設定されていない可能性があります。
