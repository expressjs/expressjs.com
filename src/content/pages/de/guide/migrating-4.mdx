...
<ul class="doklist">
  <li>
    <a href="#changes">Änderungen in Express 4</a>.
  </li>
  <li>
    <a href="#example-migration">Ein Beispiel</a> für die Migration einer Express 3 App auf Express 4.
  </li>
  <li>
    <a href="#app-gen">Upgrade auf den Express 4 App-Generator</a>.
  </li>
</ul>
...
<ul class="doklist">
  <li>
    <a href="#core-changes">Änderungen des Express-Core- und Middleware-Systems.</a> Die Abhängigkeiten von
    Connect und eingebauter Middleware wurden entfernt, daher müssen Sie selbst Middleware hinzufügen.
  </li>
  <li>
    <a href="#routing">Änderungen am Routingsystem.</a>
  </li>
  <li>
    <a href="#other-changes">Verschiedene andere Änderungen.</a>
  </li>
</ul>
...
Informationen über Routen finden Sie unter [`Router()` Dokumentation](/en/4x/api#router).
...
// Middleware spezifisch für diesen
Router. se(req, res, next) => {
  console.log('Time: ', Datum. ow());
  next();
});
// die Startseite Route
definieren. et('/', (req, res) => {
...
<table class="doktierbar" border="1">
  ...
  <tr>
    <td>Node.js</td><td>Express 4 benötigt Node.js 0.10.x oder höher und hat die Unterstützung für Node.js 0.8.x fallen gelassen.</td>
  </tr>
  <tr>
    <td markdown="1">`http.createServer()`</td>
    <td markdown="1">
      Das `http` Modul wird nicht mehr benötigt, außer du musst direkt mit ihm
      (socket.io/SPDY/HTTPS) arbeiten. Die App kann über die Funktion `app.listen()` gestartet werden.
    </td>
  </tr>
  <tr>
    <td markdown="1">`app.configure()`</td>
    <td markdown="1">
      Die Funktion `app.configure()` wurde entfernt. Benutze die Funktion `process.env.NODE_ENV` oder
      `app.get('env')` um die Umgebung zu erkennen und die App entsprechend zu konfigurieren.
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td markdown="1">
      Die Funktionalität ist nun auf das Setzen des grundlegenden Cookie-Wertes beschränkt. Benutze `res.cookie()` für hinzugefügte
      Funktionalität.
    </td>
  </tr>
</table>
...
// alle Umgebungen
app. et('port', process.env.PORT || 3000);
...
http. reateServer(app).listen(app.get('port'), () => {
  console.log('Express Server, der auf Port ' + app.get('port'));
});
```
...
```bash
$ npm installieren serve-favicon morgan method-override express-session body-parser multer errorhandler express@latest pug@latest --save
```
...
// Fehler beim Umgang mit Middleware sollten nach dem Laden der Routen
geladen werden, wenn (app. et('env') === 'development') {
  app. se(errorHandler());
}
...
var server = app.listen(app.get('port'), () => {
  debug('Express Server, der auf Port '+ server.address().port);
});
```
