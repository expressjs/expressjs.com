...
| `encoding`     | Kodierungstyp der Datei                        |                 |
| `mimetype`     | Mime-Typ der Datei                             |                 |
| `size`         | Größe der Datei in Bytes                       |                 |
| `destination`  | Der Ordner, in dem die Datei gespeichert wurde | `DiskStorage`   |
| `filename`     | Der Name der Datei innerhalb des `destination` | `DiskStorage`   |
...
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dest` oder `storage` | Wo die Dateien gespeichert werden sollen                                                                                                                                                |
...
Akzeptieren Sie eine einzelne Datei mit dem Namen `fieldname`. Die einzelne Datei wird
in `req.file` gespeichert.

#### `.array(fieldname[, maxCount])`

Akzeptieren Sie ein Array von Dateien, alle mit dem Namen `fieldname`. Optionaler Fehler wenn
...
[Node.js Fehlerbehandlung](https://web.archive.org/web/20220417042018/https://www.joyent.com/node-js/production/design/errors)
...
| `fieldSize`         | Maximale Feldwertgröße (in Bytes)                                                                    | 1MB           |
| `fields`            | Maximale Anzahl von Nicht-Dateifeldern                                                               | Unendlichkeit |
| `fileSize`          | Für mehrteilige Formulare ist die maximale Dateigröße (in Bytes)                                     | Unendlichkeit |
| `files`             | Für mehrteilige Formulare ist die maximale Anzahl von Dateifeldern                                   | Unendlichkeit |
| `parts`             | Für mehrteilige Formulare ist die maximale Anzahl von Teilen (Felder + Dateien)                      | Unendlichkeit |
...
eine schöne Fehlerseite anzeigen, indem du [den Standard-Express-Weg](/guide/error-handling).

Wenn Sie Fehler speziell von Multer auffangen möchten, können Sie die
Middleware-Funktion selbst aufrufen. Außerdem, wenn du nur [die Multer-Fehler] fangen möchtest (https://github.com/expressjs/multer/blob/main/lib/multer-error.js), du kannst die `MulterError` Klasse verwenden, die an das `multer` Objekt selbst angeschlossen ist (z. B. `err instanceof multer.MulterError`).
