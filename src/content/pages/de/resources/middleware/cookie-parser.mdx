...
`cookie-parser` does not include its own TypeScript type definitions. If you use TypeScript, also install the community-maintained types from [DefinitelyTyped](https://github.com/DefinitelyTyped/DefinitelyTyped) as a development dependency:
...
- `options` ein Objekt, das als zweite Option an `cookie.parse` übergeben wird. Siehe
  ...
  - `decode` eine Funktion, um den Wert des Cookies zu dekodieren

Die Middleware analysiert den `Cookie`-Header auf Anfrage und legt die Cookie-Daten
als Eigenschaft `req.cookies` und, wenn ein `secret` angegeben wurde, als
