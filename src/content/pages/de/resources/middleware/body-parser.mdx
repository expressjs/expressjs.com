...
before trusting. Zum Beispiel `req.body.foo.toString()` kann auf mehreren
...
_Dies behandelt nicht mehrteilige Körper_, aufgrund ihrer komplexen und typischerweise
...
`'encoding.unsupported'` und die Eigenschaft `charset` wird auf die nicht unterstützte Kodierung
gesetzt.
...
ist auf `'entity.parse.failed'` und die Eigenschaft `body` ist auf
...
auf `403` gesetzt, die Eigenschaft `type` ist auf `'entity.verify.failed'` und die Eigenschaft
...
gelesen wird. Die Eigenschaft `received` wird auf die Anzahl der empfangenen
Bytes gesetzt, bevor die Anfrage abgebrochen wurde und die Eigenschaft `expected` ist
...
`type` ist auf `'charset.unsupported'` und die Eigenschaft `charset`
...
die Eigenschaft `type` ist auf `'encoding.unsupported'` gesetzt und die Eigenschaft `encoding`
