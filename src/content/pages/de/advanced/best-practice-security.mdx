...
app.use((req, res) => {
  try {
    if (new Url(req.uery.url).host !== 'example.com') {
      return res.status(400).nd(`Nicht unterstützte Weiterleitung zum Host: ${req.query.url}`);
    }
  } catch (e) {
    return res.tatus(400).end(`Ungültige Url: ${req.query.url}`);
  }
...
```js
// letzten app.use Aufrufe direkt vor app.listen():

// custom 404
app.use((req, res, next) => {
  res.status(404). end("Tut uns leid, das kann nicht gefunden werden!");
});
