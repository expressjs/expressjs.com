...
<ul class="doclista">
  <li>
    <a href="#core-changes">Cambios a Express core y middleware.</a> Las dependencias de
    Conectar y middleware integrados fueron eliminadas, por lo que debe añadir middleware usted mismo.
  </li>
  <li>
...
// define el router
de la página de inicio. et('/', (req, res) => {
  res.send('Página inicial de aves');
});
// define la ruta
...
<table class="doctable" border="1">
  ...
  <tr>
    <td>Node.js</td>
    <td>Express 4 requiere Node.js 0.10.x o posterior y ha eliminado el soporte para Node.js 0.8.x.</td>
  </tr>
  <tr>
...
aplicaciones. se(bodyParser.json());
...
// middleware de manejo de error debe ser cargado después de la carga de las rutas
if (app. et('env') === 'development') {
  app. se(errorHandler());
}
...
var server = app.listen(app.get('port'), () => {
  debug('Servidor Express escuchando en el puerto ' + server.address().port);
});
```
