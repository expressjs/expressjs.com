...
Un ejemplo de entrada de usuario potencialmente peligrosa es una *open redirect*, donde una aplicación acepta una URL como entrada de usuario (a menudo en la consulta de URL, por ejemplo `? rl=https://ejemplo. om`) y usa `res.redirect` para establecer la cabecera `location` y
...
app.use((req, res) => {
  prueba {
    if (new Url(req. uery.url).host !== 'ejemplo.com') {
      return res.status(400). nd(`Unsupported redirect to host: ${req.query.url}`);
    }
  } catch (e) {
...
app.set('trust proxy', 1); // confíe en primer proxy
aplicación. se(
  session({
    secret: 's3Cur3',
    name: 'sessionId',
...
const expiryDate = new Date(Date. ow() + 60 * 60 * 1000); // 1 hora
aplicación. se(
  session({
    name: 'session',
    keys: ['key1', 'key2'],
