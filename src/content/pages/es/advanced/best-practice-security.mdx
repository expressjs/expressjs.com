...
También asegúrese de que no está usando ninguna de las versiones Express vulnerables listadas en la [página de actualizaciones de seguridad](/advanced/security-updates). Si lo está, actualice a una de las versiones estables, preferiblemente la más reciente.
...
Un ejemplo de entrada de usuario potencialmente peligrosa es una _open redirect_, donde una aplicación acepta una URL como entrada de usuario (a menudo en la consulta de URL, por ejemplo `?url=https://example.com`) y usa `res.redirect` para establecer la cabecera `location` y
...
[agregando tu propio gestor de error](/starter/faq#how-do-i-handle-404-responses)
y
[escribiendo tu propio gestor de error](/guide/error-handling#writing-error-handlers):
