...
| `http.createServer()`              | The `http` module is no longer needed, unless you need to directly work with it (socket.io/SPDY/HTTPS). O aplicativo pode ser iniciado usando a função `app.listen()`. |
| `app.configure()`                  | A função `app.configure()` foi removida. Use a função `process.env.NODE_ENV` ou&#xA;`app.get('env')` para detectar o ambiente e configurar o aplicativo de acordo.     |
| `json spaces`                      | A propriedade de aplicação `json spaces` está desativada por padrão no Express 4.                                                                                      |
| `req.accepted()`                   | Use `req.accepts()`, `req.acceptsEncodings()`, `req.acceptsCharsets()`, e&#xA;`req.acceptsLanguages()`.                                                                |
...
| `res.charset`                      | Removed.                                                                                                                                                               |
| `res.setHeader('Set-Cookie', val)` | A funcionalidade agora está limitada a definir o valor básico de cookie. Use `res.cookie()` for added functionality.                                                   |
