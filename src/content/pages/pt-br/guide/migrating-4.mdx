...
. se((req, res, ext) => {
  console.log('Tempo:', Data. ow());
  next();
});
// define a rota da página inicial do roteador
. et('/', (req, res) => {
  res.send('Birds home page');
});
// define a rota
roteador. et('/about', (req, res) => {
  res.send('Sobre birds');
});
...
var usuário = require('. routes/user');
...
// todos os ambientes
app. et('porta', process.env.PORT ├3000);
...
var server = app.listen(app.get('port'), () => {
  debug('Expresse servidor escutando na porta ' + server.address().port);
});
```
