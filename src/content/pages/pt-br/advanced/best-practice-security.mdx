...
app.use((req, res) => {
  tente {
    if (nova Url(req. uery.url).host !== 'example.com') {
      return res.status(400). nd(`Redirecionamento não suportado para host: ${req.query.url}`);
    }
  } capturar (e) {
    retorna! tatus(400).end(`url inválida: ${req.query.url}`);
  }
...
app.use((req, res, next) => {
  res.status(404). end("Desculpe não consegue encontrar isso!");
});

// aplicativo personalizado para manipulador de erros
. se((err, req, res, next) => {
  console.error(err.stack);
  res.status(500).send('Algo quebra!');
});
```
...
const expiryDate = new Date(Date. ow() + 60 * 60 * 1000); aplicativo // 1 hora
se(
  sessão({
    nome: 'sessão',
