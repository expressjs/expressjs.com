...
Un exemple de saisie potentiellement dangereuse de l'utilisateur est une redirection \_ouverte, où une application accepte une URL comme entrée utilisateur (souvent dans la requête URL, par exemple `?url=https://example.com`) et utilise `res.redirect` pour définir l'en-tête `location` et
...
[ajouter votre propre gestionnaire non trouvé](/starter/faq#how-do-i-handle-404-responses)
et
[écrire votre propre gestionnaire d'erreur](/guide/error-handling#writing-error-handlers) :
