...
**Note** Les données de session ne sont _pas_ enregistrées dans le cookie lui-même, juste l'ID de session.
...
**Avertissement** Le stockage de session côté serveur par défaut, `MemoryStore`, est _utile_
...
it to be saved. _Ceci a été corrigé dans PassportJS 0.3.0_
...
Chaque boutique de session _doit_ être un `EventEmitter` et implémenter des méthodes
