...
secret.get('/', (req: Request, res: Response) => {
  console.log(secret.mountpath); // /secr{*splat}t
  res.send('Admin Secret');
});
...
// methods that are invalid JavaScript variable names use bracket notation
app['m-search']('/', (req: Request, res: Response) => {
  res.send('M-SEARCH request to homepage');
});
```
...
app.get('/user/:id', (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
  console.log('although this matches');
  next();
});
...
app.param(['id', 'page'], (req: Request, res: Response, next: NextFunction, value: string) => {
  console.log('CALLED ONLY ONCE with', value);
  next();
});
...
app.get(
  '/user/:id/:page',
  (req: Request<{ id: string; page: string }>, res: Response, next: NextFunction) => {
    console.log('although this matches');
    next();
  }
);

app.get('/user/:id/:page', (req: Request<{ id: string; page: string }>, res: Response) => {
  console.log('and this matches too');
  res.end();
});
```
...
app.use('/abcd', (req: Request, res: Response, next: NextFunction) => {
  next();
});
```
...
app.use('/ab{c}d', (req: Request, res: Response, next: NextFunction) => {
  next();
});
```
...
app.use(/\/abc|\/xyz/, (req: Request, res: Response, next: NextFunction) => {
  next();
});
```
...
app.use(['/abcd', '/xyza', /\/lmn|\/pqr/], (req: Request, res: Response, next: NextFunction) => {
  next();
});
```
...
subApp.get('/', (req: Request, res: Response, next: NextFunction) => {
  next();
});
app.use(subApp);
```
...
const r1 = express.Router();
r1.get('/', (req: Request, res: Response, next: NextFunction) => {
  next();
});

const r2 = express.Router();
r2.get('/', (req: Request, res: Response, next: NextFunction) => {
  next();
});
