...
[app.use()](/en/5x/api/application#app-use) or as the argument to another router's [use()](#router-use) method.

The top-level `express` object has a [Router()](/en/5x/api/express#express-router) method that creates a new `router` object.
...
This method is similar to [app.use()](/en/5x/api/application#app-use). A simple example and use case is described below.
See [app.use()](/en/5x/api/application#app-use) for more information.
