...
`router.query()` routes HTTP `QUERY` requests, mirroring [`app.query()`](/api/application/#appquery). The `QUERY` method is gated by the runtime and requires Node.js `>=20.19.3 <21 || >=22.2.0`.
...
router.get(/^\/commits\/(\w+)(?:\.\.(\w+))?$/, function (req: Request, res: Response) {
...
router.param('user', function (req: Request, res: Response, next: NextFunction, id) {
  // try to get the user details from the User model and attach it to the request object
  User.find(id, function (err, user) {
    if (err) {
      next(err);
...
router.param('id', function (req: Request, res: Response, next: NextFunction, id) {
  console.log('CALLED ONLY ONCE');
  next();
});

router.get('/user/:id', function (req: Request<{ id: string }>, res: Response, next: NextFunction) {
  console.log('although this matches');
  next();
});
...
// customizing the behavior of router.param()
router.param(function (param, option) {
  return function (req: Request, res: Response, next: NextFunction, val) {
    if (val === option) {
      next();
    } else {
      res.sendStatus(403);
...
router.param('user_id', function (req: Request, res: Response, next: NextFunction, id) {
  // sample user, would actually fetch from DB, etc...
  req.user = {
    id: id,
    name: 'TJ',
...
router
  .route('/users/:user_id')
  .all(function (req: Request<{ user_id: string }>, res: Response, next: NextFunction) {
    // runs for all HTTP verbs first
    // think of it as route specific middleware!
    next();
  })
  .get(function (req: Request<{ user_id: string }>, res: Response, next: NextFunction) {
    res.json(req.user);
  })
  .put(function (req: Request<{ user_id: string }>, res: Response, next: NextFunction) {
    // just an example of maybe updating the user
    req.user.name = req.params.name;
    // save user ... etc
    res.json(req.user);
  })
  .post(function (req: Request<{ user_id: string }>, res: Response, next: NextFunction) {
    next(new Error('not implemented'));
  })
  .delete(function (req: Request<{ user_id: string }>, res: Response, next: NextFunction) {
    next(new Error('not implemented'));
  });
```
...
// this will only be invoked if the path starts with /bar from the mount point
router.use('/bar', function (req: Request, res: Response, next: NextFunction) {
  // ... maybe some additional /bar logging ...
  next();
});
...
```ts
import { type Request, type Response, type NextFunction } from 'express';
import { basic } from './authenticate';
...
authRouter.get(
  '/:user_id/edit',
  function (req: Request<{ user_id: string }>, res: Response, next: NextFunction) {
    // ... Edit user UI ...
  }
);
openRouter.get('/', function (req: Request, res: Response, next: NextFunction) {
  // ... List users ...
});
openRouter.get(
  '/:user_id',
