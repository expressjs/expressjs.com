...
[app.locals](/api/application/#applocals) instead.
...
[jsonp callback name](/api/application/#application-settings) setting.
...
app.get(
  '/file/:name',
  function (req: Request<{ name: string }>, res: Response, next: NextFunction) {
    const options = {
      root: path.join(__dirname, 'public'),
      dotfiles: 'deny',
      headers: {
...
app.get(
  ...
  function (req: Request<{ uid: string; file: string }>, res: Response) {
