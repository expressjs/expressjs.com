...
app.get('/user/:id', function (req: Request<{ id: string }>, res: Response) {
  res.send('user ' + req.params.id);
});
```
...
app.get('/user/:id', function (request: Request<{ id: string }>, response: Response) {
  response.send('user ' + request.params.id);
});
```
...
```mjs title="index.mjs"
import mymiddleware from './mymiddleware.mjs';

app.get('/viewdirectory', mymiddleware);
```

And the middleware module itself:

```cjs title="mymiddleware.cjs"
module.exports = function (req, res) {
  res.send('The views directory is ' + req.app.get('views'));
};
```

```mjs title="mymiddleware.mjs"
export default function (req, res) {
  res.send('The views directory is ' + req.app.get('views'));
}
```

```ts title="mymiddleware.ts"
import { type Request, type Response } from 'express';
...
greet.get('/jp', function (req: Request, res: Response) {
  console.log(req.baseUrl); // /greet
  res.send('Konnichiwa!');
});
...
app.post('/profile', function (req: Request, res: Response, next: NextFunction) {
  console.log(req.body);
  res.json(req.body);
});
```
...
app.use('/admin', function (req: Request, res: Response, next: NextFunction) {
  // GET 'http://www.example.com/admin/new?sort=desc'
  console.dir(req.originalUrl); // '/admin/new?sort=desc'
  console.dir(req.baseUrl); // '/admin'
  console.dir(req.path); // '/new'
...
app.get('/user/:id?', function userIdHandler(req: Request<{ id: string }>, res: Response) {
  console.log(req.route);
  res.send('GET');
});
```
