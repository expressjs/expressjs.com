...
> [!IMPORTANT]
> Internal links must **not** hardcode a language (`/en/...`). Write language-agnostic links
> (`/guide/routing`, `/api/express`) and they are localized to each page's language at build time.
> Because Crowdin copies links verbatim into translations, a hardcoded language would make
> translated pages point back to English. See [Internal Links](./content.md#internal-links).
